...
def hareket (bolgeler): #robot sag duvarı referans alarak kendini ortalamaya çalışacaktır
    ...
    if bolgeler['on'] > 0.7: #robotun gideceği alan kalmadığında durması için if else tanımlıyoruz burda gidecek yer kalmadığında 36 satır devreye girecektir
        ...
        if bolgeler['on_sag'] < 1.5:
            ...
            print('Donus:SAG')
            hiz=0.4
            donus=-0.4 #eksi kullanılmasın robotun hareketinin saga doğru + kullanılması sola doğru olmasını sağlar
        else:
            print('Donus:0')
            hiz=0.4
            donus=0.0
...
if __name__ == '__main__':
    ...
    rospy.init_node('otonom_iki', anonymous=True) #dugumun tanımlanması
    ...
    rospy.loginfo("Sonlandirmak için CTRL+C") #robot durdugunda veri aktarımı duruyor fakar hareketi devam ediyor bu yüzden 46 satır kullanıyoruz
